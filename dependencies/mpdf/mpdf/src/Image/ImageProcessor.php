...
namespace WP_Ultimo\Dependencies\Mpdf\Image;

use WP_Ultimo\Dependencies\Mpdf\AssetFetcher;
use WP_Ultimo\Dependencies\Mpdf\Cache;
use WP_Ultimo\Dependencies\Mpdf\Color\ColorConverter;
use WP_Ultimo\Dependencies\Mpdf\Color\ColorModeConverter;
use WP_Ultimo\Dependencies\Mpdf\CssManager;
use WP_Ultimo\Dependencies\Mpdf\Gif\Gif;
use WP_Ultimo\Dependencies\Mpdf\Language\LanguageToFontInterface;
use WP_Ultimo\Dependencies\Mpdf\Language\ScriptToLanguageInterface;
use WP_Ultimo\Dependencies\Mpdf\Log\Context as LogContext;
use WP_Ultimo\Dependencies\Mpdf\Mpdf;
use WP_Ultimo\Dependencies\Mpdf\Otl;
use WP_Ultimo\Dependencies\Mpdf\PsrLogAwareTrait\PsrLogAwareTrait;
use WP_Ultimo\Dependencies\Mpdf\SizeConverter;
use WP_Ultimo\Dependencies\Psr\Log\LoggerInterface;
class ImageProcessor implements \WP_Ultimo\Dependencies\Psr\Log\LoggerAwareInterface
{
    ...
    public function getImage(&$file, $firstTime = \true, $allowvector = \true, $orig_srcpath = \false, $interpolation = \false)
    {
        ...
        if (!$data) {
            ...
            } catch (\WP_Ultimo\Dependencies\Mpdf\Exception\AssetFetchingException $e) {
    ...
    private function gzCompress($data)
    {
        if (!\function_exists('gzcompress')) {
            throw new \WP_Ultimo\Dependencies\Mpdf\MpdfException('gzcompress is not available. install ext-zlib extension.');
        }
    ...
    private function imageError($file, $firstTime, $msg)
    {
        ...
        if ($firstTime && ($this->mpdf->showImageErrors || $this->mpdf->debug)) {
            throw new \WP_Ultimo\Dependencies\Mpdf\MpdfImageException(\sprintf('%s (%s)', $msg, \substr($file, 0, 256)));
        }
    ...
    public function processJpg($data, $file, $firstTime, $interpolation)
    {
        ...
        if ($a[2] === 'DeviceCMYK' && ($this->mpdf->restrictColorSpace === 2 || $this->mpdf->PDFA && $this->mpdf->restrictColorSpace !== 3)) {
            ...
            if (!\function_exists('gd_info')) {
                throw new \WP_Ultimo\Dependencies\Mpdf\MpdfException(\sprintf('JPG image may not use CMYK color space (%s).', $file));
            }
    ...
    public function processPng($data, $file, $firstTime, $interpolation)
    {
        ...
        if ($firstTime && ($colspace === 'DeviceRGB' || $colspace === 'Indexed') && ($this->mpdf->PDFX || $this->mpdf->restrictColorSpace === 3)) {
            ...
            if ($pngalpha) {
                if ($this->mpdf->PDFA) {
                    throw new \WP_Ultimo\Dependencies\Mpdf\MpdfException(\sprintf('PDFA1-b does not permit images with alpha channel transparency (%s).', $file));
                }
