...
use WP_Ultimo\Dependencies\Mpdf\Strict;
use WP_Ultimo\Dependencies\Mpdf\Css\TextVars;
use WP_Ultimo\Dependencies\Mpdf\Fonts\FontCache;
use WP_Ultimo\Dependencies\Mpdf\Shaper\Indic;
use WP_Ultimo\Dependencies\Mpdf\Shaper\Myanmar;
use WP_Ultimo\Dependencies\Mpdf\Shaper\Sea;
...
class Otl
{
    ...
    function applyOTL($str, $useOTL)
    {
        ...
        for ($sch = 0; $sch <= $subchunk; $sch++) {
            ...
            if (!isset($this->GDEFdata[$this->fontkey]['GSUBGPOStables'])) {
                ...
                if (!$this->ttfOTLdata) {
                    throw new \WP_Ultimo\Dependencies\Mpdf\MpdfException('Can\'t open file ' . $this->fontCache->tempFilename($this->fontkey . '.GSUBGPOStables.dat'));
                }
            } else {
    ...
    function _applyGSUBsubtable($lookupID, $subtable, $ptr, $currGlyph, $currGID, $subtable_offset, $Type, $Flag, $MarkFilteringSet, $LuCoverage, $level, $currentTag, $is_old_spec, $tagInt)
    {
        ...
        if ($Type == 1) {
            ...
            if ($SubstFormat == 1) {
                ...
                throw new \WP_Ultimo\Dependencies\Mpdf\MpdfException("GSUB Lookup Type " . $Type . " Format " . $SubstFormat . " not TESTED YET.");
            }
        } elseif ($Type == 6) {
            //===========
            // Format 1:
            ...
            throw new \WP_Ultimo\Dependencies\Mpdf\MpdfException("GSUB Lookup Type " . $Type . " not supported.");
        }
    }
    ...
    private function _applyGPOSsubtable($lookupID, $subtable, $ptr, $currGlyph, $currGID, $subtable_offset, $Type, $Flag, $MarkFilteringSet, $LuCoverage, $tag, $level, $is_old_spec)
    {
        ...
        if ($Type == 1) {
            ...
            //===========
            if ($PosFormat == 1) {
                throw new \WP_Ultimo\Dependencies\Mpdf\MpdfException("GPOS Lookup Type " . $Type . " Format " . $PosFormat . " not TESTED YET.");
            } elseif ($PosFormat == 2) {
                $CoverageTableOffset = $subtable_offset + $this->read_ushort();
                $InputClassDefOffset = $subtable_offset + $this->read_ushort();
                ...
                throw new \WP_Ultimo\Dependencies\Mpdf\MpdfException("GPOS Lookup Type " . $Type . ", Format " . $PosFormat . " not supported.");
            }
        } elseif ($Type == 8) {
            //===========
            // Format 1:
            ...
            throw new \WP_Ultimo\Dependencies\Mpdf\MpdfException("GPOS Lookup Type " . $Type . " not supported.");
        }
    }
    //////////////////////////////////////////////////////////////////////////////////
    ...
    private function _getGCOMignoreString($flag, $MarkFilteringSet)
    {
        ...
        if ($flag & 0x10) {
            throw new \WP_Ultimo\Dependencies\Mpdf\MpdfException("This font [" . $this->fontkey . "] contains MarkGlyphSets - Not tested yet");
            // Change also in ttfontsuni.php
            if ($MarkFilteringSet == '') {
                throw new \WP_Ultimo\Dependencies\Mpdf\MpdfException("This font [" . $this->fontkey . "] contains MarkGlyphSets - but MarkFilteringSet not set");
            }
