...
use WP_Ultimo\Dependencies\Mpdf\TTFontFile;
class FontWriter
{
    use Strict;
    /**
    ...
    public function writeFonts()
    {
        ...
        foreach ($this->mpdf->fonts as $k => $font) {
            ...
            if ($type === 'Type0') {
                ...
                $this->mpdf->fonts[$k]['n'] = $this->mpdf->n + 1;
                if ($this->mpdf->PDFA || $this->mpdf->PDFX) {
                    throw new \WP_Ultimo\Dependencies\Mpdf\MpdfException('Core fonts are not allowed in PDF/A1-b or PDFX/1-a files (Times, Helvetica, Courier etc.)');
                }
                $this->writer->object();
                $this->writer->write('<</Type /Font');
                ...
                throw new \WP_Ultimo\Dependencies\Mpdf\MpdfException(\sprintf('Unsupported font type: %s (%s)', $type, $name));
            }
        }
    }
