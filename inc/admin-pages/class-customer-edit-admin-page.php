...
class Customer_Edit_Admin_Page extends Edit_Admin_Page {
	...
	public function generate_customer_meta_fields() {
		...
		foreach ($custom_meta_keys as $key => $value) {
			...
			// Add delete link for orphaned fields (those without form reference)
			$delete_link = '';
			if ( ! $form) {
				...
				$delete_link = sprintf(
					' <a href="%s" class="wu-no-underline wu-inline-block wu-text-gray-600 hover:wu-text-red-600" onclick="return confirm(\'%s\')" title="%s"><span class="dashicons-wu-squared-cross"></span></a>',
					esc_url($delete_url),
					esc_js(__('Are you sure you want to delete this custom field? This action cannot be undone.', 'multisite-ultimate')),
					esc_attr__('Delete this custom field', 'multisite-ultimate')
				);
			}
